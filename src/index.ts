...
const { Converter, AirtableUtils, RemoteConnection } = function(APIKEY: string): Utilties {
	...
	*/
	...
	const customFetch: FetchObject = {
		...
		_fetch: async function<T, U>(
			...
			let result: T = null
			try {
				const res = await fetch(`${this.baseUrl}v${this.airtableVersion}/${path}`, {
		...
		_throttleRequests: async function<T extends RecordFields, U extends Object>(
			...
			while(payload.length > 0) {
				...
				let response = await this._fetch<RemoteRecord<T>, U>(path, method, round, opts)
		...
		_doUpdateRequest: async function<T extends RecordFields, U>(
			...
			if(payload) {
				...
				const result = await this._throttleRequests<T, U>(`${baseId}/${tableId}`, method, payload, opts) 
				results.push(...result)
			} else {
				const result = await this._fetch<RemoteRecord<T>, null>(`${baseId}/${tableId}`, method, null, opts)
				results.push(result)
			}
			return results
		},
		_convertRemoteRecords: function<T extends RecordFields>(
			...
			return records.map(rec => {
				...
				return {
					...
					tableId: tableId, fields: fieldValues
				}
			})
		},
		...
		*/
		get: async function<T extends RecordFields>(
			...
			let results: Record<T>[] = [], offset = ''
			while(offset !== null) {
				const response = await this._fetch<{ records: Record<T>[], offset: string }, null>(
					`${baseId}/${tableId}?${viewId ? `view=${viewId}&` : ''}${offset ? 'offset=' + offset : ''}`,
					'GET',
					null,
					{ baseId }
				)
		...
		post: async function<T extends RecordFields, U>(
			...
			return await this._doUpdateRequest<T, U>(baseId, tableId, 'POST', payload, opts)
		},
		/** Creates and sends a post request with optional body data
		 * @param this {FetchObject}
		 * @param baseId {string}
		...
		patch: async function<T extends RecordFields, U>(
			...
			return await this._doUpdateRequest<T, U>(baseId, tableId, 'PATCH', payload, opts)
		},
		/** Creates and sends a post request with optional body data
		 * @param this {FetchObject}
		 * @param baseId {string}
		...
		put: async function<T extends RecordFields, U>(
			...
			return await this._doUpdateRequest<T, U>(baseId, tableId, 'PUT', payload, opts)
		},
		/** Creates and sends a post request with optional body data
		 * @param this {FetchObject}
		 * @param baseId {string}
		...
		delete: async function<U>(
			...
			while(payload.length !== 0) {
				...
				const result = await this._fetch<{ id: RecordId, deleted: boolean }[], null>(`${baseId}/${tableId}${delRecs}`, 'DELETE')
				results.push(...result)
			}
			return results
		}
	 ...
	 * @return {Promise<CustomRecord[]>}
	 */
	async function createRemoteRecords<T extends RecordFields, U extends LockedRecordFields>(
		baseId: BaseId,
		tableId: TableId,
		fields: U[],
		mappings: Mappings
	 ...
	 * @return {Promise<CustomRecord[]>}
	 */
	async function updateRemoteRecords<T extends RecordFields, U extends LockedRecordFields>(
		baseId: BaseId,
		tableId: TableId,
		records: Record<U>[],
		mappings: Mappings
